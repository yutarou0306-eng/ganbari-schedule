    ...
    <div style={{ minHeight: "100vh", background: oceanBg, padding: "28px 16px 60px", fontFamily: "'Kaisei Decol', 'Hiragino Maru Gothic ProN', sans-serif" }}>
      <div style={{ maxWidth: 480, margin: "0 auto" }}>
        ...
        <Section id="guide-step2" emoji="🌟" title="2. スケジュールを作る">
          ...
          <br />
          作ったスケジュールをあとから確認・閲覧したいときは、そのスケジュールが紐づいているスタンプ帳を開いて、中から探す形になります。スタンプ帳のリンクをブックマークやホーム画面に追加しておくと、毎回探しやすくなります。
          <br />
          <br />
          一度その端末でスケジュールを開くと、次回からはトップページの「見る」「完了」の一覧にもそのスケジュールが表示されるようになり、スタンプ帳を経由しなくても直接開けるようになります。
          <br />
          <br />
          保護者の方のスマホ、お子さんのタブレットなど、複数の端末それぞれで同じリンクを開けば、その端末でも同じように確認・操作できます。1つの端末に限定されるものではないので、家族みんなで見守ることができます。
        </Section>
        ...
        <Section id="guide-step5" emoji="🥚" title="5. 卵が育つ・進化する">
          ...
          <br />
          <br />
          達成率が100%になると、日数やスタンプ数に関係なく、育ったキャラはマスターまで成長し、LVも最大（Lv.20）になります。マスターまで育てば「⚗️配合」にも使えます。
          <br />
          <br />
          ただし、「ファミリアカードを獲得！」という正式な演出と、スタンプ帳の「集めたカード」一覧に永続的なカードとして残るのは、スケジュールが30日以上・必要スタンプ50個以上の場合だけです。それより短い・少ないスケジュールが100%になった場合は、見た目・LV・配合できる点はすべて同じまま「育成中」のカードとして表示され続けます。
        </Section>
        ...
        <Section id="guide-parent" emoji="🔒" title="保護者だけができること">
          スケジュール画面右上の🔒（または鍵アイコン）をタップし、確認画面で解除すると、以下ができます。
          <br />
          <br />
          ...
          <br />
          <br />
          また、スタンプ帳から「新しいスケジュールを作る」ときも、そのスタンプ帳を作った時に設定した保護者用暗証番号の入力が必要です（お子さんが自分でどんどん新しいスケジュールを作れてしまわないようにするためです）。
          <br />
          <br />
